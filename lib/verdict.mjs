...
export function summarize(result) {
  ...
  if (result.unanimous) return `${base} — unanimous`;

  if (result.dissenters.length > 0) {
    const verb = result.dissenters.length === 1 ? 'dissents' : 'dissent';
    return `${base} — ${result.dissenters.join(', ')} ${verb}`;
  }

  // No dissenters but not unanimous: the majority carried against abstentions
  // rather than against an opposing vote.
  return `${base} — ${result.abstainers.join(', ')} abstain${
    result.abstainers.length === 1 ? 's' : ''
  }`;
}
