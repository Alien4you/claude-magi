...
/** Render one cue's tones. Mirrors renderTones()/envelopeAt() in sound.mjs. */
function playCue(name) {
  ...
  for (const tone of tones) {
    ...
    const dur = tone.dur;
    const gain = (tone.gain ?? 1) * 0.16;
    const type = tone.wave === 'saw' ? 'sawtooth' : tone.wave || 'square';
    ...
    for (const offset of voices) {
      ...
      osc.type = type;
      osc.frequency.setValueAtTime(tone.freq + offset, start);
      if (tone.to) osc.frequency.linearRampToValueAtTime(tone.to + offset, start + dur);

      if (tone.env === 'gate') {
        // Fast edges, flat sustain: an alarm horn holds its level.
        const edge = Math.min(0.012, dur / 4);
        amp.gain.setValueAtTime(0.0001, start);
        amp.gain.linearRampToValueAtTime(gain, start + edge);
        amp.gain.setValueAtTime(gain, start + dur - edge);
        amp.gain.linearRampToValueAtTime(0.0001, start + dur);
      } else {
        amp.gain.setValueAtTime(0.0001, start);
        amp.gain.exponentialRampToValueAtTime(gain, start + Math.min(0.02, dur / 4));
      ...
      if (tone.tremolo) {
        ...
        lfo.frequency.setValueAtTime(tone.tremolo, start);
        depth.gain.setValueAtTime(gain * 0.22, start);
...
function run() {
  ...
  PLAN.order.forEach((step, i) => {
    later(() => {
      ...
      playCue(step.tone === 'approve' ? 'affirm' : step.tone === 'reject' ? 'negate' : 'withhold');
  ...
  const dissenting = PLAN.order.some((s) => s.dissent);
  const finalAt = settle + PLAN.order.length * 1400 + (dissenting ? 3300 : 700);
