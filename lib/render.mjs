...
import { CUES, TIMING, cueDuration } from './sound.mjs';
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  @keyframes flicker-animation { 0% {} 50% { background: #000; color: #000; border-color: #000; } }
  .panel[data-state="thinking"] > .inner { animation: flicker-animation .25s infinite step-end; }
 ...
 * sine with a linear attack, a linear release, and an optional exponential
 * decay between them.
 */
function playCue(name) {
  ...
  for (const tone of tones) {
    ...
    if (tone.decay) {
      // Amplitude x e^(-rate*t), floored so the ramp stays legal.
      amp.gain.exponentialRampToValueAtTime(
        Math.max(0.0001, level * Math.exp(-tone.decay * (dur - release))),
        start + dur - release,
      );
    } else {
      amp.gain.setValueAtTime(level, start + Math.max(attack, dur - release));
    }
...
function run() {
  ...
  PLAN.order.forEach((step, i) => {
    later(() => {
      panels.get(step.agent).setAttribute('data-state', 'thinking');
      playCue('tick');
    }, 260 * i);
  });

  // Deliberation: the pulse train loops on its own 442 ms period. Cut it at a
  // period boundary, never mid-pulse.
  const thinkFrom = 700;
  for (let c = 0; c < THINK_CYCLES; c++) later(() => playCue('think'), thinkFrom + c * cycle);

  // The verdict lands 1.436 s after the last cycle's first pulse onset, which
  // leaves 0.275 s of silence. think and a verdict never overlap.
  let at = thinkFrom + (THINK_CYCLES - 1) * cycle + T.verdictAfterThinkOnset * 1000;

  PLAN.order.forEach((step) => {
    const cue = step.tone === 'approve' ? 'agree' : step.tone === 'reject' ? 'reject' : 'withhold';
    const onset = at;
    ...
    at = onset + PLAN.durations[cue] * 1000 + gap;

    if (step.dissent) {
      const klaxonAt = onset + PLAN.durations[cue] * 1000 + gap;
      later(() => playCue('klaxon'), klaxonAt);
      at = klaxonAt + PLAN.durations.klaxon * 1000 + gap;
    }
  });
  ...
  later(() => {
    ...
    playCue(PLAN.approved ? 'approved' : 'rejected');
